...
from auth import create_user, authenticate_user, login_required, is_authenticated, get_current_user
...
def index():
    user_id = get_user_id()
    return render_template('index.html', user_id=user_id, user_email=get_current_user())
...
def start_convo():
    try:
        ...
        # Create user directory with email-safe name
        safe_user_id = user_id.replace('@', '_at_').replace('.', '_dot_')
        user_dir = f"user_data/{safe_user_id}"
        os.makedirs(user_dir, exist_ok=True)
...
def restart_server():
    try:
        ...
        if user_id in user_processes and server_id in user_processes[user_id]:
            ...
            # Restart the server
            safe_user_id = user_id.replace('@', '_at_').replace('.', '_dot_')
            user_data_dir = f"user_data/{safe_user_id}"
